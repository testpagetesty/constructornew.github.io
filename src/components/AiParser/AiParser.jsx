...
ВНИМАНИЕ! СТРОГО следуйте этому формату с пустыми строками между каждым элементом контактов!
...
[Полный адрес в стандартном формате страны, ОБЯЗАТЕЛЬНО ВКЛЮЧАЯ НАЗВАНИЕ СТРАНЫ в конце адреса:
- Для России: город, улица, дом, индекс, Россия (например: "Москва, ул. Большая Семёновская, д. 40, 107023, Россия")
- Для США: улица, город, штат, zip, USA (например: "2500 N Forest Rd, Getzville, NY 14068, USA")
- Для Германии: улица, почтовый код, город, Deutschland (например: "Mehringdamm 33, 10961 Berlin, Deutschland")
- Для Франции: улица, почтовый код, город, France (например: "123 Avenue de Flandre, 75019 Paris, France")
- Для Великобритании: номер дома, улица, город, почтовый код, United Kingdom (например: "45 Deansgate, Manchester M3 2AY, United Kingdom")
- Для Канады: улица, город, провинция, почтовый код, Canada (например: "1055 W Georgia St, Vancouver, BC V6E 3P3, Canada")
- Для Австралии: улица, пригород, штат, почтовый код, Australia (например: "200 Queen St, Melbourne VIC 3000, Australia")
- Для Испании: улица, номер, город, почтовый код, España (например: "Carrer de Balmes 145, Barcelona 08008, España")
- Для Италии: улица, номер, город, почтовый код, Italia (например: "Via Torino 61, Milano 20123, Italia")
- Для других стран: полный адрес согласно местным стандартам адресации, с ОБЯЗАТЕЛЬНЫМ указанием страны в конце адреса]
...
Пример ПРАВИЛЬНОЙ структуры на русском (заменить на выбранный язык):
Контакты
...
ВНИМАНИЕ! ОБЯЗАТЕЛЬНО следуйте этому формату с пустыми строками между каждым элементом контактов!
=== КОНЕЦ РАЗДЕЛА ===\n\n`;
    }
...
ВНИМАНИЕ! Строго следуйте этому формату с пустыми строками между каждым элементом!
=== КОНЕЦ РАЗДЕЛА ===
...
6. КРИТИЧЕСКИ ВАЖНО ДЛЯ АДРЕСОВ:
   - Использовать только РЕАЛЬНЫЕ адреса, которые существуют на Google Maps
   - ОБЯЗАТЕЛЬНО указывать название страны в конце каждого адреса
   - НЕ использовать: известные улицы/места (Тверская, Арбат, Oxford Street, Times Square, Broadway, Champs-Élysées), правительственные здания, достопримечательности
